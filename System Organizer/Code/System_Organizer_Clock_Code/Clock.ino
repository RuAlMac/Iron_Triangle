...
//    inputKey = sysClockKeypad.getKey();
    ...
    if (inputKey) {
      ...
      if (inputKey == 'Y') { //check to make sure input time is valid
        checkTimeInput[0] = ( (hour[0] * 10) + hour[1] );
        checkTimeInput[1] = ( (minute[0] * 10) + minute[1] );

        Serial.println(checkTimeInput[0]);
        Serial.println(checkTimeInput[1]);

        if ( (checkTimeInput[0] >= 0) && (checkTimeInput[0] < 24) && (checkTimeInput[1] >= 0) && (checkTimeInput[1] < 60) ) {
          sysClockSet = true;
        }
        else {
          lcd.setCursor(11, 1);
...
void runSysClock() {
  if (sysClockSet == true) {
    ...
    if ( (currentMillis == startMillis + 1000) || (firstRun == true) ) {
      firstRun = false;
      ...
      if ( (hour[0] == 2) && (hour[1] == 4) ) {
        ...
        Serial.println("*****Clock error: Millis() function reset*****");
        firstRun = true;
      }
