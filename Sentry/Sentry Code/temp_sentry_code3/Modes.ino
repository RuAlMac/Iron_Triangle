...
void listenToComms() {
  ...
  if ( (rComm[0] == 3) && (rComm[1] == 7) && (rComm[2] == 2) && (rComm[3] == 8) ) { //rComm[] == {3,7,2,8}
    newEvent(12);
  }
...
void receiveSettings() {
  if ( radio.available() ) {              //checking for incoming message
    ...
    delay(10);
    radio.read(&transmitArray, sizeof(transmitArray));
    delay(10);
  }

  if ( (transmitArray[0] != 0) && (transmitArray[0] != 3) ) {
    lcd.setCursor(12, 3);
    lcd.print("  ");
    lcd.setCursor(12, 3);
    lcd.print(transmitArray[0]);
  }

  /*Serial.print("transmitArray["); //for use in debugging this goofy function
  ...
  sSettings[ transmitArray[0] ] = transmitArray[1];

  if ( (rComm[0] == 1) && (rComm[1] == 1) && (rComm[2] == 1) && (rComm[3] == 1) ) { //rComm[] == {1,1,1,1}
    Serial.println("Finished receiving settings");
    ...
    for (int y = 0; y <= 20; y++) {
      ...
      Serial.print("------> ");
      Serial.print("sSettings[");
      Serial.print(y);
      Serial.print("] = ");
      Serial.print(sSettings[y]);
      Serial.print("\n");
    }
